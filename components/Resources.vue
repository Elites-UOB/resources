<template>
    <div w="full" flex="~ col gap-3"  v-if="resourcesStore.getFilteredResources?.length > 0">
        <Resource v-for="resource in resourcesStore.getFilteredResources" :resource="resource" :key="resource.id" />
        <Skeleton v-show="loading" />
    </div>
    <div v-else text="pw dark:pwd lg" flex="~ col gap-4" items-center justify-center bg="p dark:pd" w="auto sm:full"
